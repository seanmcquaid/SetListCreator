...
describe("<ClientEditSetListPage/>", () => {
  ...
  test("Add Bandleader Comment", async () => {
    ...
    fireEvent.change(screen.getByTestId("CommentsTextInput"), {
      target: { value: "Bandleader Comment Here" },
    });
    expect(screen.getByTestId("CommentsTextInput").value).toEqual(
      "Bandleader Comment Here"
    );

    fireEvent.click(screen.getByTestId("Add CommentButton"));

    await waitFor(() =>
      expect(screen.getByText("Bandleader Comment Here")).toBeInTheDocument()
    );
  ...
  test("Send Edited setlist error", async () => {
    ...
    jest.spyOn(axios, "patch").mockRejectedValueOnce({
      response: { data: { ...editCompletedSetListResponse } },
    });
