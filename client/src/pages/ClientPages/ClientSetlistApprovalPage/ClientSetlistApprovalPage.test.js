...
describe("<ClientSendSetListPage/>", () => {
  ...
  test("Send Client Comments and Approval redirects to client home when succesful", async () => {
    ...
    fireEvent.change(screen.getByTestId("Is This Approved?Dropdown"), {
      target: { value: "Yes" },
    });
    expect(screen.getByTestId("Is This Approved?Dropdown").value).toEqual(
      "Yes"
    );

    fireEvent.change(screen.getByTestId("Add CommentsTextInput"), {
      target: { value: "No Music" },
    });
    ...
    await waitFor(() =>
      expect(screen.queryByText("loadingSpinner")).toBeNull()
    );
