...
describe("<ClientEditSongPage/>", () => {
  ...
  test("Song Info Loads", async () => {
    ...
    expect(screen.getByTestId("Song TypeDropdown").value).toEqual(
      "doNotPlaySong"
    );
  });
  ...
  test("Song Info Load Error Displays", async () => {
    ...
    expect(
      screen.getByText("There was a problem getting the song info")
    ).toBeInTheDocument();
  });
  ...
  test("Edit Song submit with new information redirects the user to the client home and sees the edited song", async () => {
    ...
    fireEvent.change(screen.getByTestId("Song TypeDropdown"), {
      target: { value: "requestedSong" },
    });
    expect(screen.getByTestId("Song TypeDropdown").value).toEqual(
      "requestedSong"
    );
    ...
    jest
      .spyOn(axios, "patch")
      .mockResolvedValueOnce({ data: { ...editClientSongActionResponse } });
