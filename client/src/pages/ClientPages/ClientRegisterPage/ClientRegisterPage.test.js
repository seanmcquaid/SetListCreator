...
describe("<ClientRegisterPage/>", () => {
  ...
  test("Loading Spinner appears when you first enter the page", async () => {
    ...
    jest
      .spyOn(axios, "get")
      .mockResolvedValueOnce({ data: { ...getBandleadersResponse } });
  ...
  test("Error displays when there is a problem with getting bandleaders", async () => {
    ...
    expect(
      screen.getByText("There was a problem getting Bandleaders, please reload")
    ).toBeInTheDocument();
  });
  ...
  test("Successfully register user - redirected to client home", async () => {
    ...
    fireEvent.change(screen.getByTestId("Select Your BandleaderDropdown"), {
      target: { value: selectedBandleader },
    });
    expect(screen.getByTestId("Select Your BandleaderDropdown").value).toEqual(
      selectedBandleader
    );
  ...
  describe("Unsucessfully register user", () => {
    test("Passwords don't match", async () => {
      ...
      fireEvent.change(screen.getByTestId("Confirm PasswordTextInput"), {
        target: { value: wrongConfirmPassword },
      });
      expect(screen.getByTestId("Confirm PasswordTextInput").value).toEqual(
      ...
      expect(
        screen.getByTestId("Select Your BandleaderDropdown").value
      ).toEqual(selectedBandleader);
    ...
    test("User already is registered - error message displays", async () => {
      ...
      await waitFor(() =>
        expect(screen.getByText("Register Error Here")).toBeInTheDocument()
      );
    });
  });
});
