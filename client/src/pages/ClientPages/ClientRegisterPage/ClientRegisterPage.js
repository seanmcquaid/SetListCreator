...
const ClientRegisterPage = () => {
  ...
  useEffect(() => {
    if (isMounted.current) {
      ...
      axios
        .get(`${apiHost}/users/getBandleaders`, config)
        .then((response) => {
          const timer = setTimeout(() => {
            const bandLeadersArray = response.data.bandleaders.map(
              (bandleader) => bandleader.username
            );
            let initialArray = [""];
        ...
        .catch(() => {
          const timer = setTimeout(() => {
            setNewErrorMessage(
              "There was a problem getting Bandleaders, please reload"
            );
            setIsLoading(false);
            source.cancel();
          }, 1500);
  ...
  const clientRegisterSubmitHandler = useCallback(
    (event) => {
    ...
    <div className={styles.clientRegisterContainer}>
      <div className={styles.textContainer}>
        ...
        <Text>
          ...
          <Link className={styles.registerLink} to="/clientLogin">
            Here
          </Link>
        </Text>
      ...
      <form
        ...
        onSubmit={clientRegisterSubmitHandler}
      >
        <Input
          name="username"
          title="Username"
