...
exports.getClientSongs = async (req, res, next) => {
  ...
  return await UsersModel.getUserInfo(clientId)
    .then(async (clientInfo) => {
      ...
      return await ClientSongListModel.getSongs(userInfo.username).then(
        async (clientSongs) => {
    ...
    .then(async (clientInfo) => {
      ...
      return await ClientSongListModel.getSongs(userInfo.username).then(
        async (clientSongs) => {
          ...
          return await BandleaderSongListModel.getSongs(username).then(
            async (bandleaderSongs) => {
              const suggestedSetList = bandleaderSongs.filter(
                (bandleaderSong) => {
                  const isBandleaderSongInClientDoNotPlayList = doNotPlaySongsList.find(
                    (doNotPlaySong) =>
                      doNotPlaySong.songname === bandleaderSong.songname
                  );
                  return isBandleaderSongInClientDoNotPlayList
                    ? null
                    : bandleaderSong;
                }
              );

              const additionalClientRequests = requestedSongsList.filter(
                (requestedSong) => {
                  const isRequestedSongInSuggestedSetlist = suggestedSetList.find(
                    (suggestedSong) =>
                      suggestedSong.songname === requestedSong.songname
                  );
                  return isRequestedSongInSuggestedSetlist
                    ? null
                    : requestedSong;
                }
              );

              return await SetListsModel.getSetList(userInfo.username).then(
                async (setListInfo) =>
                  await res.status(200).send({
                    suggestedSetList,
                    additionalClientRequests,
                    clientComments:
                      setListInfo.length > 0 && setListInfo.clientcomments
                        ? setListInfo.clientcomments
                        : [],
                  })
              );
            }
...
exports.postCompletedSetList = async (req, res, next) => {
  ...
  return await UsersModel.getUserInfo(clientId)
    .then(async (clientInfo) => {
      ...
      return await SetListsModel.addSetList(
        clientName,
        bandleaderName,
        completedSetList,
        bandleaderComments
...
exports.getClientSetListInfo = async (req, res, next) => {
  ...
  return await UsersModel.getUserInfo(clientId)
    .then(async (userInfo) => {
      ...
      return await SetListsModel.getSetList(clientInfo.username).then(
        async (setListInfo) => {
          const {
            clientname,
            bandleadername,
    ...
    .catch(
      async (err) =>
        await res.status(500).send({
          errorMessage:
            "There was a problem getting the client Set List information",
        })
    );
};
...
exports.editCompletedSetList = async (req, res, next) => {
  ...
  return await UsersModel.getUserInfo(clientId)
    .then(async (clientInfo) => {
      ...
      return await SetListsModel.editSetList(
        clientName,
        bandleaderName,
        completedSetList,
        bandleaderComments
...
exports.deleteAllSongsAndSetList = async (req, res, next) => {
  ...
  try {
    ...
    return res.status(500).send({
      errorMessage:
        "There was a problem deleting all songs and set lists for a bandleader",
    });
  }
};
