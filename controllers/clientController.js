...
exports.postAddSong = async (req, res, next) => {
  ...
  return await ClientSongListModel.addSong(
    ...
    .then(async (addedSong) => {
      return await UsersModel.getUserInfo(id).then(async (clientInfo) => {
            ...
            return await SetListsModel.getSetList(userInfo.username).then(
              async (setListInfo) =>
                await res.status(200).send({
                  ...
                  clientApproved:
                    setListInfo.length > 0
                      ? setListInfo[0].clientapproved
                      : false,
                })
            );
          }
...
exports.getSongs = async (req, res, next) => {
  ...
  return await ClientSongListModel.getSongs(username)
    .then(async (clientSongs) => {
      return await UsersModel.getUserInfo(id).then(async (userInfo) => {
        const { requestedSongsList, doNotPlaySongsList } = clientSongs;

        return await SetListsModel.getSetList(userInfo[0].username).then(
          async (setListInfo) =>
            await res.status(200).send({
              requestedSongsList,
              doNotPlaySongsList,
              ...
              clientApproved:
                setListInfo.length > 0 ? setListInfo[0].clientapproved : false,
            })
        );
      });
    })
...
exports.deleteAllSongsAndSetList = async (req, res, next) => {
  ...
  try {
    ...
    return res.status(500).send({
      errorMessage:
        "There was a problem deleting all songs and set lists for a client",
    });
  }
};
